"""Evaluate the transit-route save workflow and its approval boundary."""
...
class EvalFailure(AssertionError):
    """A human-readable transit HITL evaluation failure."""
...
async def evaluate(storage_path: Path) -> None:
    """Run, approve, and verify one transit itinerary save workflow."""
    ...
    async with travel_agent_session() as (agent, tools):
        ...
        require(bool(interrupts), "公共交通行程保存前必须产生 __interrupt__")
        ...
        require(
            pending_args.get("travel_mode") == "transit",
            "公共交通行程的 travel_mode 必须是 transit",
        )
        require(
        ...
        notes = str(pending_args.get("notes") or "")
        require("步行" in notes, "公共交通行程备注应包含步行信息")
        require("线路" in notes or "校巴" in notes, "备注应包含乘坐线路")
        print("[PASS] transit_route_and_save_are_orchestrated")

        exists_before_approval = await asyncio.to_thread(storage_path.exists)
        require(not exists_before_approval, "批准前不应创建行程文件")
        print("[PASS] transit_save_is_interrupted_before_write")
        ...
        require(len(lines) == 1, "批准一次应只保存一条公共交通行程")
        ...
        print("[PASS] approved_transit_action_is_written_once")

        final_answer = render_user_response(resumed["messages"])
        print(f"\n最终回答：\n{final_answer}\n")
        require("保存" in final_answer, "最终回答应提及保存结果")
        require(
            "公交路线数据来源：高德地图 Web服务 API" in final_answer,
            "最终回答应保留公共交通路线数据来源",
        )
        require(
            not any(
        ...
        print("[PASS] transit_final_answer_respects_data_boundaries")
...
async def main() -> None:
    ...
    try:
        with TemporaryDirectory(
            prefix="smart-travel-agent-transit-hitl-"
        ) as temp_dir:
